...
Operation* DedupeAndHoistConstantPass::BuildIdentity(Operation* input,
  ...
  // Concat `identity` prefix with the const op name to avoid name collision.
  // TODO(rdzhabarov): Generate trully unique name for the Identity node
  // in the given scope.
  if (auto const_name = input->getAttrOfType<StringAttr>(name_id)) {
    state.addAttribute(
        name_id,
        StringAttr::get(mlir_context, "identity/" + const_name.getValue()));
  }

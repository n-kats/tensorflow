...
class Subgraph {
 public:
  ...
  static TfLiteStatus VisitSplitNode(
    ...
    int output1_idx = node->outputs->data[0];
    const TfLiteTensor& output1_tensor = tensors[output1_idx];
    TF_LITE_ENSURE_STATUS(CheckTensorFloat32OrQUInt8Type(
        delegate, logging_context, output1_tensor, output1_idx, node_index));
    TF_LITE_ENSURE_STATUS(CheckTensorNonDynamicAllocation(
        logging_context, output1_tensor, output1_idx, node_index));

    int output2_idx = node->outputs->data[1];
    const TfLiteTensor& output2_tensor = tensors[output2_idx];
    TF_LITE_ENSURE_STATUS(CheckTensorFloat32OrQUInt8Type(
        delegate, logging_context, output2_tensor, output2_idx, node_index));
    TF_LITE_ENSURE_STATUS(CheckTensorNonDynamicAllocation(
        logging_context, output2_tensor, output2_idx, node_index));
    ...
    for (int i = 0; i < NumOutputs(node); i++) {
      int output_idx = node->outputs->data[1];
      ...
      for (int d = 0; d < NumDimensions(&input_tensor); d++) {
        if (d == split_dim) {
          ...
          TF_LITE_ENSURE_STATUS(CheckTensorsDimensionMatch(
              logging_context, input_tensor, output1_tensor, d, node_index,
              "SPLIT"));
        }
      }
    }
    ...
    if (subgraph != nullptr) {
      const xnn_status status =
          xnn_define_split_even2(subgraph, split_dim,
                                 /*input_id=*/xnnpack_tensors[input_idx],
                                 /*output1_id=*/xnnpack_tensors[output1_idx],
                                 /*output2_id=*/xnnpack_tensors[output2_idx],
                                 /*flags=*/0);
      if (status != xnn_status_success) {

...
class SplitTester {
 public:
  ...
  inline SplitTester& SplitDimension(int32_t split_dim) {
    EXPECT_EQ(0, split_dim_.size());
    EXPECT_LT(0, InputShape().size());
    split_dim_.push_back(split_dim);
    return *this;
  }
  ...
  std::vector<int32_t> SplitDimension() const { return split_dim_; }
  ...
  std::vector<int32_t> OutputShape() const {
    ...
    int32_t split_dim = SplitDimension().front();
    split_dim += split_dim < 0 ? InputShape().size() : 0;
    EXPECT_LE(0, split_dim);
    EXPECT_EQ(0, output_shape[split_dim] % 2);
